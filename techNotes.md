...
- open crestron
- turn off the skype button (should be red)
- check that projector turned on automatically
- open zoom
- join at meeting > 957 2271 8820 (name 1A4, passcode cElegans)
...
- can remind speaker that what's on the projector is a second screen so if they need to they can use their cursor but will have to move it off to the side
- can suggest they keep their mic pack somewhere visible so we can see if the battery goes out (spare battery in the cupboard)
- can tell them don't worry about any chat notifications as jonathan will monitor it and tell them if there's anything important
